...
/** Main function. This does not take any parameters. It assumes
    the events are something -> tau+ tau-, then tau -> rho nu_tau
			   ...
			   int nparams, double *params)
{
    ...
    //make temporary variables to store tau+
    double taup_px=0;
    double taup_py=0;
    double taup_pz=0;
    double taup_e=0;
    ...
    taup_e+=d_pi0_plus.GetX0();        // is it mass or energy 

    taum_px+=d_pi0_minus.GetX1();
    taum_py+=d_pi0_minus.GetX2();
    taum_pz+=d_pi0_minus.GetX3();
    taum_e+=d_pi0_minus.GetX0();        // is it mass or energy 
    ...
    // this make no sense for TauSpinner examples:
    ...
    // we do not use this boos because it make no sense for LHC applications:
    // may be used with true tau momenta for the sake of reference to refined studies.
    ...
    */
    //boost to the (generated) tau's frames. It is an option!
    /*
