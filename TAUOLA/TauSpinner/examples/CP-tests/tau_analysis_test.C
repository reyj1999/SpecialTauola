			 ...
			 int nparams, double *params)
{
    ...
    double pi_plus[3]={0};
    double pi_minus[3]={0};
    double tau_nu_plus[3]={0};
    double tau_nu_minus[3]={0};
    double tau_prod[3]={0};
	...
	tau_minus[2]+=d4.GetX3();
      ...
      // Set production vert using HepMC
      //      cout << "prodvert X: " << hX->production_vertex()->position().x () << "Y: "<< hX->production_vertex()->position().y () <<"z" <<  hX->production_vertex()->position().z () << "t" <<  hX->production_vertex()->position().t () << endl;
	...
	pi_plus[2]=d4.GetX3();
      ...
      //fill tau+ neutrino or tau- neutrino array
      if(part->GetPDGId()==-16){
        tau_nu_minus[0]=d4.GetX1();
	tau_nu_minus[1]=d4.GetX2();
	tau_nu_minus[2]=d4.GetX3();
      }
      if(part->GetPDGId()==16){
	tau_nu_plus[0]=d4.GetX1();
	tau_nu_plus[1]=d4.GetX2();
	tau_nu_plus[2]=d4.GetX3();
      }
    ...
    // Check if weight has been provided as parameter
    /*
        double WT = 1.0;
        if( nparams>=1 ) WT = params[0];
    else {
...
/*	sprintf(plotname,"theta");
    ...
    /*** Acollinarity **/
    //calculate the angle between the pi+ and pi- (in Higgs rest frame)
    /*    double delta = acos(dot_product(tau_nu_plus,tau_nu_minus)/(magnitude(tau_nu_plus)*magnitude(tau_nu_minus)));
    char *plotname = new char[30];
    double WT = 1.0;
      if( nparams>=1 ) WT = params[0];
      else {
    ...
    /*** Acoplanarity (theta) **/
    //calculate the angle between  pi+ and tau+
    double projection_plus = dot_product(tau_nu_plus,tau_plus)/(magnitude(tau_plus)*magnitude(tau_plus));

    char *plotname = new char[30];

    //calculate the 3-vector for the pi+ transverse to the tau+
    double tau_nu_plus_pt[3];
    tau_nu_plus_pt[0]  = tau_nu_plus[0]-(projection_plus*tau_plus[0]);
    tau_nu_plus_pt[1]  = tau_nu_plus[1]-(projection_plus*tau_plus[1]);
    tau_nu_plus_pt[2]  = tau_nu_plus[2]-(projection_plus*tau_plus[2]);

    //calculate the angle between  pi- and tau-
    double projection_minus = dot_product(tau_nu_minus,tau_minus)/(magnitude(tau_minus)*magnitude(tau_minus));

    //calculate the 3-vector for the pi- transverse to the tau-
    double tau_nu_minus_pt[3];
    tau_nu_minus_pt[0]  = tau_nu_minus[0]-(projection_minus*tau_minus[0]);
    tau_nu_minus_pt[1]  = tau_nu_minus[1]-(projection_minus*tau_minus[1]);
    tau_nu_minus_pt[2]  = tau_nu_minus[2]-(projection_minus*tau_minus[2]);

    //calculate the angle between the pi+ transverse and pi- transverse
    //but this only gives the angle from 0 to pi.
    double theta = acos(dot_product(tau_nu_plus_pt,tau_nu_minus_pt)/(magnitude(tau_nu_plus_pt)*magnitude(tau_nu_minus_pt)));

    //to get the angle between 0 and 2 pi, use the normal to the pi+ pi- transverse pair
    //if the normal is in the direction of the tau-, set theta = 2pi - theta.
    double m3[3];
    normalised_cross_product(tau_nu_plus_pt,tau_nu_minus_pt,m3);
    double theta_sign = dot_product(m3,tau_minus)/magnitude(tau_minus);
    if(theta_sign>0)
      theta=2*M_PI-theta;
