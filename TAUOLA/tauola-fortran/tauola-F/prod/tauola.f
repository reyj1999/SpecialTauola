...
C-----------------------------------------------------------------------
C  Initialize RChL Currents
C  Dummy routine for compatibility with new updates of TAUOLA
C
C  Added 25.Jul.2012
C-----------------------------------------------------------------------
      SUBROUTINE INIRCHL(FLAG)
      INTEGER FLAG

      IF(FLAG.NE.0) THEN
        WRITE(*,25) FLAG
 25     FORMAT(1X, "TAUOLA IniRChL: Fatal error, FLAG=",I2," but RChL currents missing")
        WRITE(*,*) "       in loaded version of TAUOLA-FORTRAN library."
        STOP
      ENDIF
